# PDF Markdown Converter — Ultimate Stress-Test Document
This document is intentionally complex. It contains italic prose, bold headings, bold-italic combinations, nested tables, multi-column data, inline code, fenced code blocks in multiple languages, checklists (checked & unchecked), and edge-case formatting designed to break naive converters.
...
| Library    | Language    | Stars | License    | Description                                                                 |
| :--------- | :---------- | :---- | :--------- | :-------------------------------------------------------------------------- |
| FastAPI    | Python      | 72k   | MIT        | OpenAPI auto-docs; excellent typing support                                 |
...
| NestJS     | TypeScript  | 66k   | MIT        | Angular-style DI; opinionated structure                                     |
...
Normal text flows into **bold text** and then *italic text* and then ***bold-italic text*** and back to normal. This sentence tests mid-word transitions: ***unexpected*** and ***semiautomatic*** and ***overloaded***.
...
*An entirely italic paragraph follows. It contains a nested **bold** island inside italic text, which should produce bold-italic output: ***this is bold-italic***. Then the italic continues normally.*
### 4.2 Inline Code Inside Prose
Call the function `fetch_paginated()` with a `page_size` argument. The return type is `list[dict[str, Any]]`. Do not confuse `None` with `null`; they are different types. The environment variable `DATABASE_URL` must be set before calling `migrate()`.
...
> WARNING: Running `DROP TABLE users` in production is irreversible. Ensure you have a verified backup before proceeding. This action will trigger a P0 incident if executed without a change-management ticket.
### 4.4 Deeply Nested Checklist
Below is a checklist where some items have sub-items (indented). Converters should preserve the indent level as nested list items.
- [x] Backend services healthy
  - [x] API gateway responding
  - [x] Auth service token refresh tested
...
Ampersand: A & B. Angle brackets: `<tag>` and `</tag>`. Quotes: “double" and ‘single’. Em dash: mdash — ndash – hyphen-minus -. Ellipsis: waiting... vs proper: …. Copyright: © Registered: ® Trademark: ™.
Superscripts: x² y³ z°. Subscripts: H₂O CO₂. Currency: $. Mathematical: + =. Numbers: 1,234.56, 1.234,56 EUR, 1,23,456 INR.
### 5.2 Long Unbreakable Tokens
The following is a long identifier that may challenge line-wrapping: `com.example.platform.services.authentication.providers.oauth2.handlers.RefreshTokenExchangeHandler`. And a long URL: `https://api.example.com/v2/organisations/org_O1HXYZ/members?include=roles,permissions&page_size=100&cursor=eyJpZC16MTEzfQ`.
...
- [x] Checked boxes render as `[x]` and unchecked as `[ ]`
...
- [ ] Alternating background not lost (acceptable)
